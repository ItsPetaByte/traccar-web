...
const MobileGroupStatusCard = ({ position, onClose, desktopPadding = 0 }) => {
  ...
  const groupStatus = mobileGroupStatuses.find(
    (item) => item?.value == position?.['groupStatus']
  );

  const groupNumber = position?.['groupNumber'];
  const groupRole = position?.['functionName'];
  const groupPhoneNumber = position?.['mobilePhone'];
